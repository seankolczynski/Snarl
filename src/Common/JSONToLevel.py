...
def managerMaker(jsonStuff):
    ...
    while index < len(ptList):
        adverse = Adversary(3, index, name_list[index], monster_types[index - 2], 2)
...
def layout_parser(layout, username):
    ...
    for tile in actor_tiles:
        ...
        actorJSON.append(
            {"type": act.get_ctype(), "name": act.get_name(), "position": translate_to_xy(tile.get_position())})
...
def playersToJSON(game):
    ...
    for player in players:
        if player.is_alive():
            playerJSON.append(
                {"type": "player", "name": player.get_name(), "position": translate_to_xy(player.get_char_position())})
    return playerJSON
...
def adversary_view(grid):
    layout = []
    for inde in range(len(grid)):
        layout.append([])
    for y in range(len(grid[0])):
        for x in range(len(grid)):
            layout[x].append(grid[x][y].num_val())
    return layout
